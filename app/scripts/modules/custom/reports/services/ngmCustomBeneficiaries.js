...
angular.module('ngmReportHub')
    ...
    .factory('ngmCustomBeneficiaries', ['$http', '$filter', '$timeout', 'ngmAuth', 'ngmCustomConfig','ngmCustomLists', 'ngmClusterVulnerablePopulations', 'ngmClusterDetails', 'ngmClusterHelperNgWash',
        function ($http, $filter, $timeout, ngmAuth, ngmCustomConfig, ngmCustomLists, ngmClusterVulnerablePopulations, ngmClusterDetails, ngmClusterHelperNgWash) {

            // beneficairies
            var ngmCustomBeneficiaries = {
                ...
                updateBeneficiairesBreakdown: function (beneficiary) {
                    ...
                    $timeout(function () {
                        ...
                        // calc totals
                        ngmCustomBeneficiaries.updateBeneficiaires(beneficiary);
                    }, 100);
                },
                ...
                updateBeneficiaires: function (beneficiary) {
                    ...
                    $timeout(function () {
                        ...
                        ngmCustomBeneficiaries.updateTotalTransferedAmount(beneficiary);
                    }, 100);
                },
                ...
                addBeneficiary: function (project, beneficiaries) {
                    ...
                    var defaults = ngmCustomBeneficiaries.defaults;
                ...
                addAddHocBeneficiary: function (project, location, beneficiaries) {
                    ...
                    // inserted
                    var inserted = ngmCustomConfig.getCustomBeneficiariesConfigAttribute(project.definition.report_type_id,project.definition.version);
                    var length = beneficiaries.length;
                    if (length) {
                        var b = angular.copy(beneficiaries[length - 1]);
                        delete b.id;
                ...
                setActivity: function (project, $parent, $index, beneficiary) {
                    ...
                    if (beneficiary.activity_type_id && project.definition.activity_type.length) {
                        ...
                        if (selected && selected.length) {
                            ...
                            // defaults
                            ngmCustomBeneficiaries.setDefaultBeneficiaryAttributes(beneficiary, project.definition.admin0pcode);
                            ...
                            // set form
                            ngmCustomBeneficiaries.setBeneficiariesInputs(project.lists, $parent, $index, beneficiary);
                        }
                    }
                ...
                setBeneficiaries: function (project, type, $parent, $index, beneficiary) {
                    ...
                    angular.forEach(ngmCustomBeneficiaries.merge_keys, function (key, i) {
                        beneficiary[key] = ngmCustomBeneficiaries.form[$parent][$index][key];
                    });

                    // clear
                    if (beneficiary.vulnerable_populations && beneficiary.vulnerable_populations.length) {
                        beneficiary.vulnerable_populations = [{}];
                        ngmClusterVulnerablePopulations.setList(ngmCustomBeneficiaries.form[$parent][$index].vulnerable_populations, $parent, $index, 0, '', [{}]);
                    }

                    // clear
                    if (ngmCustomBeneficiaries.form[$parent][$index]['display_details'] && ngmCustomBeneficiaries.form[$parent][$index]['details'] && ngmCustomBeneficiaries.form[$parent][$index]['details'].length) {
                        beneficiary.details = [{}];
                        ngmClusterDetails.setList(ngmCustomBeneficiaries.form[$parent][$index].details, $parent, $index, 0, '', [{}]);
                    }


                    // clear cash / package / units
                    // angular.forEach( defaults.cash_package_units, function ( i, key ) {
                    // 	if ( !ngmCustomBeneficiaries.form[ $parent ][ $index ][ key ] && key.indexOf( '_name' ) === -1 ) {
                    ...
                    // if (project.definition.admin0pcode === 'AF' && ngmCustomBeneficiaries.form[$parent][$index]['beneficiary_category_type_id'] && !beneficiary.beneficiary_category_id){
                ...
                setLocationsForm: function (lists, locations) {
                    ...
                    angular.forEach(locations, function (location, location_index) {
                        // for each location
                        ngmCustomBeneficiaries.setBeneficiariesForm(lists, location_index, location.beneficiaries);
                    });
                ...
                setBeneficiariesForm: function (lists, $parent, beneficiaries) {
                    ...
                    if (beneficiaries && beneficiaries.length) {
                        ...
                        angular.forEach(beneficiaries, function (beneficiary, $index) {
                            ngmCustomBeneficiaries.setBeneficiariesInputs(lists, $parent, $index, beneficiary);
                        });
                    }
                ...
                setBeneficiariesInputs: function (lists, $parent, $index, beneficiary) {
                    ...
                    // add form holder
                    if (!ngmCustomBeneficiaries.form[$parent]) {
                        ngmCustomBeneficiaries.form[$parent] = [];
                    }
                    ...
                    if (beneficiary.display_indicator && beneficiary.indicator_id) {
                        ngmCustomBeneficiaries.form[$parent][$index] = $filter('filter')(lists.activity_indicators, { indicator_id: beneficiary.indicator_id, activity_detail_id: beneficiary.activity_detail_id, activity_description_id: beneficiary.activity_description_id, activity_type_id: beneficiary.activity_type_id, cluster_id: beneficiary.cluster_id }, true)[0];
                    }
                    // beneficiary.display_activity_detail
                    else if (beneficiary.display_activity_detail && beneficiary.activity_detail_id) {
                        ngmCustomBeneficiaries.form[$parent][$index] = $filter('filter')(lists.activity_details, { activity_detail_id: beneficiary.activity_detail_id, activity_description_id: beneficiary.activity_description_id, activity_type_id: beneficiary.activity_type_id, cluster_id: beneficiary.cluster_id }, true)[0];
                    }
                    // beneficiary.activity_description_id
                    else if (beneficiary.activity_description_id) {
                        ngmCustomBeneficiaries.form[$parent][$index] = $filter('filter')(lists.activity_descriptions, { activity_description_id: beneficiary.activity_description_id, activity_type_id: beneficiary.activity_type_id, cluster_id: beneficiary.cluster_id }, true)[0];
                    }
                    // reset form
                    else if (beneficiary.activity_type_id) {
                        ngmCustomBeneficiaries.form[$parent][$index] = angular.copy(ngmCustomBeneficiaries.defaults.form);
                    }

                    // set default form on activity missing
                    if (typeof ngmCustomBeneficiaries.form[$parent][$index] === 'undefined') {
                        ngmCustomBeneficiaries.form[$parent][$index] = angular.copy(ngmCustomBeneficiaries.defaults.form);
                    }

                    // set details
                    if (!ngmCustomBeneficiaries.form[$parent][$index]['display_details'] &&
                        ngmCustomBeneficiaries.form[$parent][$index]['details'] &&
                        ngmCustomBeneficiaries.form[$parent][$index]['details'].length) {
                        // set details from activities.csv
                        beneficiary.details = ngmCustomBeneficiaries.form[$parent][$index].details;
                    } else {
                        // remove if exists
                    ...
                    if (ngmCustomBeneficiaries.form[$parent][$index]) {
                        ngmCustomBeneficiaries.form[$parent][$index].display = ngmCustomBeneficiaries.showFormInputs(beneficiary, ngmCustomBeneficiaries.form[$parent][$index]);
                    }
                    // if beneficiary.response exist then check ngmCustomBeneficiaries.form[$parent][$index]['exist']
                    if (beneficiary.response && beneficiary.response.length > 0) {

                        if (!ngmCustomBeneficiaries.form[$parent][$index]['response']) {
                            temp_list = []
                        } else {
                            var temp_list = ngmCustomBeneficiaries.form[$parent][$index]['response'];
                        }
                        var count_missing = 0;
                        ...
                        if (count_missing > 0) {
                            // set ngmCustomBeneficiaries.form[$parent][$index]['response'] same as temp list if some of response_id is missing
                            ngmCustomBeneficiaries.form[$parent][$index]['response'] = temp_list;
                        }
                    }
                    ...
                    if (ngmCustomBeneficiaries.form[$parent][$index] && ngmCustomBeneficiaries.form[$parent][$index]['mpc_transfer_category_id'] && ngmCustomBeneficiaries.form[$parent][$index]['mpc_grant_type_id']) {
                        //set default value for 'mpc_transfer_category_id' 'mpc_grant_type_id'
                        // ngmCustomBeneficiaries.setDefaultValueBeneficiary($parent, $index, 'mpc_transfer_category_id', beneficiary, 'transfer_category_id', 'transfer_category_name');
                        // ngmCustomBeneficiaries.setDefaultValueBeneficiary($parent, $index, 'mpc_grant_type_id', beneficiary, 'grant_type_id', 'grant_type_name');
                        ngmCustomBeneficiaries.setDefaultValueBeneficiary(ngmCustomBeneficiaries.form[$parent][$index]['mpc_transfer_category_id'], beneficiary, 'transfer_category_id', 'transfer_category_name');
                        ngmCustomBeneficiaries.setDefaultValueBeneficiary(ngmCustomBeneficiaries.form[$parent][$index]['mpc_grant_type_id'], beneficiary, 'grant_type_id', 'grant_type_name');
                    }

                    if (ngmCustomBeneficiaries.form[$parent][$index] && ngmCustomBeneficiaries.form[$parent][$index]['beneficiary_category_type_id']) {
                        ngmCustomBeneficiaries.setDefaultValueBeneficiary(lists.beneficiary_categories, beneficiary, 'beneficiary_category_id', 'beneficiary_category_name');
                    }
                ...
                setDefaultValueBeneficiary: function (list, beneficiary, key, name) {
                    ...
                    // 	if (ngmCustomBeneficiaries.form[$parent][$index][field].length < 2 && ngmCustomBeneficiaries.form[$parent][$index][field]) {
                    // 		 disabled = true;
                    // 	}
                    // set value
                    $timeout(function () {
                        ...
                        if (l.beneficiaries) {
                            ...
                            if (l.beneficiaries.length) {
                                angular.forEach(l.beneficiaries, function (b) {
                                    if (!ngmCustomBeneficiaries.rowSaveDisabled(project, b)) {
                                        rowComplete++;
                                    }
                                });
                            }
