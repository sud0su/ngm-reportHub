...
/**
 * @name ngmReportHub.factory:ngmCustomValidation
 * @description
 * # ngmCustomValidation
            ...
            'ngmAuth', function ($http, $filter, $timeout, ngmClusterBeneficiaries, ngmAuth) {
                ...
                var ngmCustomValidation = {
                    ...
                    project_details_valid: function (project) {
                        ...
                        // valid
                        ngmCustomValidation.project_details_valid_labels = [];

                        if (!project.project_title) {
                            ngmCustomValidation.project_details_valid_labels.push('ngm-project-name');
                        }
                        if (!project.project_start_date || project.project_start_date === 'Invalid date') {
                            ngmCustomValidation.project_details_valid_labels.push('ngm-start-date');
                        }
                        if (!project.project_end_date || project.project_end_date === 'Invalid date') {
                            ngmCustomValidation.project_details_valid_labels.push('ngm-end-date');
                        }
                        ...
                        if (!project.project_budget_currency) {
                            // ngmCustomValidation.project_details_valid_labels.push('ngm-project-budget');
                            ngmCustomValidation.project_details_valid_labels.push('ngm-project_budget_currency');
                        }
                        ...
                        if (!project.project_description) {
                            ngmCustomValidation.project_details_valid_labels.push('ngm-project-description');
                        }

                        // // console.log('project_details_valid');
                        // // console.log(ngmCustomValidation.project_details_valid_labels.length);

                        // if NO labels details valid
                        return !ngmCustomValidation.project_details_valid_labels.length;
                    },

                    projectDescription: function (project) {
                        ngmCustomValidation.project_description_valid_labels = [];
                        ...
                        if (!project.project_description || (/^\s*$/).test(project.project_description)) {
                            ngmCustomValidation.project_description_valid_labels.push('ngm-project-description');
                        }
                        return !ngmCustomValidation.project_description_valid_labels.length;
                    ...
                    activity_type_valid: function (project) {
                        ...
                        // valid
                        ngmCustomValidation.activity_type_valid_labels = [];

                        // activity types?
                        if (typeof project.activity_type_check === 'undefined' || project.activity_type.length < 1) {
                            ngmCustomValidation.activity_type_valid_labels.push('ngm-activity_type');
                        ...
                        // // console.log(ngmCustomValidation.activity_type_valid_labels.length);

                        // if NO labels activities valid
                        return !ngmCustomValidation.activity_type_valid_labels.length;
                    },
                    ...
                    project_donor_valid: function (project) {
                        ...
                        // valid
                        ngmCustomValidation.project_donor_valid_labels = [];

                        // donor
                        if (!project.project_donor_check || (project.project_donor.length < 1)) {
                            ngmCustomValidation.project_donor_valid_labels.push('ngm-project_donor');
                        }

                        // // console.log('project_donor_valid_labels');
                        // // console.log(ngmCustomValidation.project_donor_valid_labels.length);

                        // if NO labels activities valid
                        return !ngmCustomValidation.project_donor_valid_labels.length;
                    },
                    ...
                    targetLocationsValidate: function (project, detail) {
                        ngmCustomValidation.targetLocationsValidatelabel = [];
                        // new validation
                        var elements = [];
                        var notDetailOpen = [];
                        locationRow = 0;
                        ...
                        if (!project.target_locations.length) {
                            ngmCustomValidation.targetLocationsValidatelabel.push('#ngm-target_locations')
                        ...
                        angular.forEach(project.target_locations, function (l, i) {
                            locationRow++;
                            result = ngmCustomValidation.targetLocationValidate(l, i, detail);
                        ...
                        if (locationRow !== locationRowComplete && notDetailOpen.length > 0) {
                            ...
                            ngmCustomValidation.targetLocationsValidatelabel.push(elements[0])
                            $timeout(function () {
                                angular.forEach(notDetailOpen, function (indexLocation) {
                                    x = indexLocation;
                                    resultRelabel = ngmCustomValidation.targetLocationValidate(project.target_locations[x], x, detail);
                                })
                    ...
                    targetBeneficiariesValidate: function (project, detail) {
                        ngmCustomValidation.targetBenfeciariesValidatelabel = [];
                        // new validation
                        var elements = [];
                        var notDetailOpen = [];
                        ...
                        angular.forEach(project.target_beneficiaries, function (b, i) {
                            ...
                            result = ngmCustomValidation.targetBeneficiaryValidate(b, i, detail, project);
                            angular.merge(elements, result.divs);
                        ...
                        if (targetBeneficiaryRow !== targetBeneficiaryRowComplete && notDetailOpen.length > 0) {
                            ...
                            ngmCustomValidation.targetBenfeciariesValidatelabel.push(elements[0]);
                            $timeout(function () {
                                angular.forEach(notDetailOpen, function (indexBeneficiary) {
                                    x = indexBeneficiary;
                                    resultRelabel = ngmCustomValidation.targetBeneficiaryValidate(project.target_beneficiaries[x], x, detail, project);
                                })
                    ...
                    validate: function (project, detailBeneficiaries, detailLocations, display_modal) {
                        ...
                        var scrollDiv;
                        var a = ngmCustomValidation.project_details_valid(project);
                        var b = ngmCustomValidation.activity_type_valid(project);
                        var c = ngmCustomValidation.project_donor_valid(project);
                        var desc = ngmCustomValidation.projectDescription(project);
                        var d = ngmCustomValidation.targetBeneficiariesValidate(project, detailBeneficiaries);
                        var e = project.admin0pcode !== 'AF' ? ngmCustomValidation.target_locations_valid(project) : ngmCustomValidation.targetLocationsValidate(project, detailLocations);
                        // locations invalid!
                        if (!e) {
                            $('#ngm-target_locations').addClass('error');
                            scrollDiv = $('#ngm-target_locations');
                            if (project.admin0pcode === 'AF') {
                                scrollDiv = $(ngmCustomValidation.targetLocationsValidatelabel[0]);
                            }
                        ...
                        if (!d) {
                            ...
                            scrollDiv = $(ngmCustomValidation.targetBenfeciariesValidatelabel[0]);//$('#ngm-target_beneficiaries');
                        ...
                        // donor
                        angular.forEach(ngmCustomValidation.project_donor_valid_labels, function (l, i) {
                        ...
                        // activity types
                        angular.forEach(ngmCustomValidation.activity_type_valid_labels, function (l, i) {
                            $('label[for=' + l + ']').addClass('error');
                            scrollDiv = $('#ngm-activity_type_label');
                        });

                        // project description
                        angular.forEach(ngmCustomValidation.project_details_valid_labels, function (l, i) {
                            $('label[for=' + l + ']').addClass('error');
                    ...
                    validateBeneficiaries: function (location, detail, admin0pcode, hrp_project_status) {
                        ...
                        angular.forEach(location, function (l, i) {
                            angular.forEach(l.beneficiaries, function (b, j) {
                                beneficiaryRow++;
                                result = ngmCustomValidation.validateBeneficiary(b, i, j, detail, admin0pcode, hrp_project_status);
                                angular.merge(elements, result.divs);
                        ...
                        if (beneficiaryRow !== beneficiaryRowComplete && notDetailOpen.length > 0) {
                            ...
                            $timeout(function () {
                                angular.forEach(notDetailOpen, function (indexbeneficiaries) {
                                    ...
                                    resultRelabel = ngmCustomValidation.validateBeneficiary(location[x].beneficiaries[y], x, y, detail, admin0pcode, hrp_project_status);
                                });
                    ...
                    validateBeneficiariesAdhoc: function (location, detail, prop) {
                        ...
                        angular.forEach(location, function (l, i) {
                            angular.forEach(l.beneficiaries, function (b, j) {
                                beneficiaryRow++;
                                result = ngmCustomValidation.validateBeneficiaryAdhoc(b, i, j, detail, prop);
                                angular.merge(elements, result.divs);
